...
$multi_img=$get->get_multi_photo($id);
$multiphotos="";
for ($i=0;$i<=count($multi_img)-1;$i++){
	if($i!=count($multi_img)-1){
		$multiphotos.= $multi_img[$i]['src'].",";
	}else{
		$multiphotos.= $multi_img[$i]['src'];
	}
		...
		<form class="content" action="../update/<?=$data['id'];?>" method="post" enctype="multipart/form-data" >
			...
			<div class="col-lg-8 row">
				...
				<div class="form-group col-md-8">
					...
					<select class="form-control select2 select2-hidden-accessible" name="category" style="width: 100%;" tabindex="-1" aria-hidden="true">
						...
						foreach ($sections as $section){
							if($data['section_id']== $section['section_id']){
				...
				<div class="form-group col-md-8">
					...
					<br>
					<input class="form-control" type="text" placeholder="Alt" value="<?=$data['alt']?>" name="alt">
				</div>

				<div class="form-group col-md-10">
					<div class="row">
						<i> Project Multi Photo</i>
						<br><br>
						<button id="btnupload" class="pull-right">upload</button>
						<input hidden="true" id="fileupload" type="file" name="image[]" multiple >
						<br>
						<div class="progress" style="height: 2px;">
							<div class="progress-bar" role="progressbar" id="uploader_status" aria-valuenow="0"  aria-valuemin="0" aria-valuemax="100">

							</div>
						</div>
						<div id="divleft" class="timeline-body">
							<?php
							foreach ($multi_img as $img){
								 echo '<img width="150" height="100" class="margin" src="'.base_url('uploads/thumbs/images').'/'.$img['src'].'">';
							}
							?>
				...
				<div class="row form-group col-md-11">
					...
					<script type="text/javascript">
						base_url="<?=base_url()?>";
						dirs = "<?php echo Directory_Manager('projects','projects').'/' ?>";
			...
			<aside class="box box-primary" style="border-top: none">
				...
				<input type="hidden" name="section" id="section" value="projects">
				<input type="hidden" name="project_photo_src" id="project_photo_src" value="<?=$data['img'];?>">
				<input type="hidden" value="<?=$multiphotos;?>" name="multi_photos" id="hidden_multi_photo">
